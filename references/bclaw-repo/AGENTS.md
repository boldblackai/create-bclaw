...
- Secrets are **SSM SecureString** parameters under a static `/bclaw/KEY`
  namespace (NOT `/<claw>/KEY`), written by the user in setup Phase 3
  (piranesi pattern). The static prefix lets the deployer IAM policy pin
  `parameter/bclaw/*` for least privilege. Not stack-owned, so they survive
  stack updates/deletes. 5 always-required params: `SLACK_BOT_TOKEN`,
