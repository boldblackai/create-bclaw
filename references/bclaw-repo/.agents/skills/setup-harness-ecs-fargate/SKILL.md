...
The claw name is always `bclaw` — it is hardcoded, not collected. The
`/bclaw/` SSM namespace, the IAM policy's `parameter/bclaw/*` scope, and the
KMS alias `alias/bclaw-ssm` all assume this name; changing it would break every
static prefix. `$CLAW_NAME` is still used as a shell variable in commands below
but is always `bclaw`.
...
```bash
CLAW_NAME=bclaw                              # hardcoded — see note above
...
> **One claw per account.** Because the namespace is static, two claws in the
> same AWS account would write to the same `/bclaw/*` parameters and collide.
> That is the trade-off for the deterministic IAM scope.
